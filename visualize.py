...
def doubler(array):
    doubled = np.zeros([2 * i for i in array.shape])
    if len(array.shape) == 1:
        for i in range(array.shape[0]):
            doubled[2 * i] = array[i]
            doubled[2 * i + 1] = array[i]
    elif len(array.shape) == 2:
        for i in range(array.shape[0]):
            for j in range(array.shape[1]):
                doubled[2 * i][2 * j] = array[i][j]
                doubled[2 * i + 1][2 * j] = array[i][j]
                doubled[2 * i][2 * j + 1] = array[i][j]
                doubled[2 * i + 1][2 * j + 1] = array[i][j]
    return doubled
